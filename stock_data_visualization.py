...
print("Your sample stock chart is plotting...")
...
def graph_data(stock):
    ...
    source_code = None
    try:
        source_code = urllib.request.urlopen(stock_price_url, timeout=30).read().decode()
    except Exception as e:
        print("Unable to download raw data, please check network connectivity \n".format(e))
        return
    ...
    for line in split_source:
        ...
        if len(split_line) == 7:
            if 'values' not in line and 'labels' not in line and 'Date' not in line:
    ...
    date, openp, highp, lowp, closep, aclosep, volume = np.loadtxt(stock_data,
                                                                   delimiter=',',
                                                                   unpack=True,
                                                                   converters={0: bytespdate2num('%Y-%m-%d')})
    ...
    plt.show()
    fig.savefig('stock_data.png', facecolor=fig.get_facecolor())


graph_data('Sample Stock Data')
